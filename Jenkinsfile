            ...
            steps {
                sshagent(credentials: ['jenkins-ssh']) {
                    sh '''
                        ...
                        sleep 10
                        ssh root@docker 'docker ps'
                        ssh root@docker 'docker exec $(docker ps --filter name=unemployment_py-wsgi -q) /bin/rm -rf staticfiles/*'
                        ...
                        sleep 10
                        ssh root@beta 'docker ps'
                        ssh root@beta 'docker exec $(docker ps --filter name=unemployment_py-wsgi -q) /bin/rm -rf staticfiles/*'
